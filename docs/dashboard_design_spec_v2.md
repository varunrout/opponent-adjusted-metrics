...
- **Coverage-gated, not full `oam_core` coverage.** A shot only gets a CxG value where it falls inside `oam_ml`'s v3 **test-split** coverage — the dashboard deliberately shows only the `split = 'test'` rows, not every row in the prediction tables. Where a shot falls outside that split, show xG only. No CxG placeholder, dash, zero, or "N/A" — a value that looks like it should be there but isn't reads as broken, not as honestly out-of-scope.
  - **Correction (confirmed against live BigQuery, 26 Aug 2026):** `oam_ml.cxg_event_v3_predictions` and `cxg_plus_v3_predictions` actually contain all three splits, not just `test`. Real row counts: `cxg_event` — train 10,890 / validation 2,420 / **test 2,427** / total 15,737. `cxg_plus` — train 2,780 / validation 590 / **test 590** / total 3,960. The dashboard's displayed coverage (test split only, per the code's `WHERE split = 'test'` filter) is therefore ~6.5× smaller than the full prediction volume in these tables — a deliberate scope choice to avoid showing predictions on rows the model was trained on, not a data gap. See v3 §12 for the open decision on whether to widen this. Both tables also already carry `statsbomb_xg` and `is_goal` columns, so an xG-vs-CxG comparison needs no join back to `oam_core`.
...
1. **CxG results (any version) may be shown in Models/Stories/Analysis, and CxG values may be shown in the Explore zone, only once an honest, documented comparison against the StatsBomb baseline exists — including where CxG underperforms.** Status language must not overclaim ("Evaluated," not "Validated" or "Production," until a production inference pipeline exists — see gate 2). *Status: unblocked — CxG v3 training is complete on both tracks, and the comparison (including CxG trailing StatsBomb on log_loss on both tracks) is documented in §11 below, satisfying the honest-comparison condition. This reframes the gate from "must beat the baseline" to "must be honestly compared to the baseline" — CxG v1 losing to StatsBomb was already framed in §1 as a credibility signal for the data-scientist persona, not something to hide, and v3 landing in the same place doesn't change that. **Update (26 Aug 2026): the public-facing Stories writeup now exists** — `/stories/cxg-v3-honest-comparison`, linked from Overview, Models, and the Stories index — so the earlier "still-pending follow-up" note above is stale; both the disclosure and the writeup are live. See §4a for where/how CxG may now appear in the Explore zone.*
2. **`oam_serving` must be populated with player-level CxG/CxA values** before: Track B (build-your-own quadrant scatter, originally scoped for Analysis) can be built at all. *Status: blocked — `oam_serving` has zero tables, confirmed live (26 Aug 2026).*
3. **Signed URL generation for `gs://` chart URIs must exist** before: Analysis tab charts render as actual images/iframes instead of literal text paths. *Status: closed (26 Aug 2026) — `gcs_signing.py` is built (impersonated-credentials `signBlob` via `roles/iam.serviceAccountTokenCreator` on `oam-pipeline-sa`, degrading to `None`/the raw `gs://` path on any failure) and the IAM grant was applied to `oam-pipeline-sa` on 26 Aug 2026. Verified by code review: `web/app/analysis/page.tsx` renders `<iframe src={chart.signed_html_url}>` whenever the API returns a non-null `signed_html_url`, and falls back to the raw `gs://` text only when it's null — the frontend correctly reflects whichever state the backend returns. This session had no live Cloud Run/GCS access to confirm a signed URL succeeds end-to-end in the deployed environment; that live confirmation is still worth doing once, but the code on both ends is correct and consistent with the grant being in place.*
4. **A TTL cache layer on `ServingStore`'s `oam_core` queries must exist** before: this dashboard link is shared with real external visitors at any scale beyond Varun's own testing. *Status: closed — implemented in `bigquery_store.py` (`cachetools.TTLCache`, 300s TTL, one cache per query method) with a passing regression test (`tests/api/test_bigquery_store_caching.py`).*
5. **`RoleProvider`'s `/v1/me` call needs a timeout guard** before: this reliability bug can be considered closed. *Status: closed — implemented in `web/components/shell/RoleProvider.tsx` (9s timeout race against `getMe`, falls back to guest and always resolves), with two passing tests (`role-provider-timeout.test.tsx`, `role-provider-guest.test.tsx`).*
