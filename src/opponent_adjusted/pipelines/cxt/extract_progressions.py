...
def _compute_xt_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["macro_zone_start"] = df.apply(
        lambda r: get_macro_zone(r["start_x"] if pd.notna(r["start_x"]) else 60, 
                                  r["start_y"] if pd.notna(r["start_y"]) else 40), 
        axis=1
    )
    df["macro_zone_end"] = df.apply(
        lambda r: get_macro_zone(r["end_x"] if pd.notna(r["end_x"]) else 60,
                                  r["end_y"] if pd.notna(r["end_y"]) else 40),
        axis=1
    )
