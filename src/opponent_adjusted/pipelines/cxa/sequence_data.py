...
def _get_shot_info(shots_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return shots_df[available].copy().rename(columns={
...
def _build_single_sequence(
    ...
    seq_data = {
        ...
        "competition_id": group["competition_id"].iloc[0] if "competition_id" in group.columns else None,
        ...
        "shot_id": group["sequence_shot_id"].iloc[0] if "sequence_shot_id" in group.columns else None,
    }
    ...
    for pos in range(1, k + 1):
        ...
        if f"{prefix}id" not in seq_data:
            for col in ["id", "player_id", "player_name", "recipient_id", "recipient_name",
                       "start_x", "start_y", "end_x", "end_y",
                       "start_xt", "end_xt", "xt_delta",
                       "is_progressive", "is_into_box", "is_cross", "is_through_ball",
                       "sequence_xA", "minute", "second"]:
...
def _add_derived_features(df: pd.DataFrame, k: int) -> pd.DataFrame:
    ...
    if "pass1_end_x" in df.columns and "shot_x" in df.columns:
        # Find the earliest pass position
        first_pass_cols = {
            f"pass{i}_start_x": f"pass{i}_start_y"
    ...
    if "pass1_minute" in df.columns and "shot_minute" in df.columns:
        ...
        df["sequence_duration_seconds"] = (
            (df["shot_minute"] - df["sequence_start_minute"]) * 60 +
...
def aggregate_player_sequences(df: pd.DataFrame, k: int = 3) -> pd.DataFrame:
    ...
    for pos in range(1, k + 1):
        ...
        agg = subset.groupby([player_col, name_col]).agg(
            count=(player_col, "count"),
            total_xt=pd.NamedAgg(column=xt_col, aggfunc="sum") if xt_col in df.columns else (player_col, "count"),
            total_xA=pd.NamedAgg(column=xa_col, aggfunc="sum") if xa_col in df.columns else (player_col, "count"),
            goals_assisted=(
                "is_goal", 
                lambda x: x.sum() if pos == 1 else 0
            ) if "is_goal" in df.columns else (player_col, "count"),
        ).reset_index()
