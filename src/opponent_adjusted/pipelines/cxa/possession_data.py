...
def build_possession_dataset(
    ...
    possession_df = grouped.agg(
        ...
        max_xt_single_pass=("xt_delta", "max") if "xt_delta" in df.columns else ("pass_id", "count"),
        ...
        # Pass types
        num_progressive=("is_progressive", "sum") if "is_progressive" in df.columns else ("pass_id", "count"),
        num_into_box=("is_into_box", "sum") if "is_into_box" in df.columns else ("pass_id", "count"),
        num_crosses=("is_cross", "sum") if "is_cross" in df.columns else ("pass_id", "count"),
        num_through_balls=("is_through_ball", "sum") if "is_through_ball" in df.columns else ("pass_id", "count"),
        num_complete=("is_complete", "sum") if "is_complete" in df.columns else ("pass_id", "count"),
    ...
    possession_df["duration_seconds"] = (
        (possession_df["end_minute"] - possession_df["start_minute"]) * 60 +
        (possession_df["end_second"] - possession_df["start_second"])
    )
...
def _link_shot_outcomes(
    ...
    shot_outcomes = shots_with_possession.groupby(
