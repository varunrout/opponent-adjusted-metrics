...
def add_xt_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "is_complete" in df.columns:
        ...
        df.loc[~df["is_complete"].fillna(False).astype(bool), "xt_delta"] = -df.loc[
            ~df["is_complete"].fillna(False).astype(bool), "start_xt"
        ]
        df.loc[~df["is_complete"].fillna(False).astype(bool), "end_xt"] = 0.0

    logger.info(f"  Mean xT delta: {df['xt_delta'].mean():.6f}")
    logger.info(
        f"  Positive xT passes: {(df['xt_delta'] > 0).sum():,} ({(df['xt_delta'] > 0).mean()*100:.1f}%)"
    )
