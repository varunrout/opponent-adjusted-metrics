...
def add_opponent_features(
    ...
    # Create lookup key
    zone_profiles["lookup_key"] = zone_profiles["team_id"].astype(str) + "_" + zone_profiles["zone_id"]
    ...
    df["opponent_zone_rating"] = df["lookup_key"].map(zone_rating_map).fillna(df["opponent_global_rating"])
    df["opponent_zone_block_rate"] = df["lookup_key"].map(zone_block_map).fillna(df["opponent_global_block_rate"])
...
def add_game_context_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"  Added game context features")
    return df
...
def add_zone_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Zone characteristics
    zone_names = {
        1: "DEF_CENTRAL", 2: "DEF_WIDE_L", 3: "DEF_WIDE_R",
        4: "MID_CENTRAL", 5: "MID_WIDE_L", 6: "MID_WIDE_R",
        7: "ATT_CENTRAL", 8: "ATT_WIDE_L", 9: "ATT_WIDE_R",
    }
    ...
    logger.info(f"  Added zone features")
    return df
...
def add_pressure_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"  Added pressure features")
    return df
...
def add_action_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"  Added action features")
    return df
