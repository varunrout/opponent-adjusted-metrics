...
def compute_xa_plus_passes(
    ...
    goal_sequences = df[df['is_goal'] == True]
    ...
    assist_credit = passes_long[passes_long['is_assist'] == True]['xa_plus'].sum()
    logger.info(f"  Credit to assists: {assist_credit:.1f} ({assist_credit/max(credits_assigned,1)*100:.1f}%)")
