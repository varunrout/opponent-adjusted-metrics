...
def _trace_action_sequence(
    ...
    poss_actions = poss_actions.sort_values(
        ["minute", "second"], 
    ...
    before_shot["dist_to_shot"] = np.sqrt(
        (before_shot["end_x"] - shot_x)**2 + 
        (before_shot["end_y"] - shot_y)**2
    )
    ...
    key_pass_time = (key_pass["minute"], key_pass["second"])
    
    # Get all actions before the shot, after or at key pass time
    before_shot = poss_actions[
        (poss_actions["minute"] < shot_minute) |
        ((poss_actions["minute"] == shot_minute) & 
         (poss_actions["second"] < shot_second))
    ]
        ...
        for _, candidate in candidates.iterrows():
            ...
            if candidate["event_type"] == "Pass":
                if candidate.get("recipient_player_id") == current_player:
                    ...
                    dist = np.sqrt(
                        (end_loc[0] - current_start[0])**2 +
                        (end_loc[1] - current_start[1])**2
                    )
...
def _add_sequence_features(df: pd.DataFrame, k: int) -> pd.DataFrame:
    ...
    if "action1_end_x" in df.columns:
        df["key_pass_distance_to_goal"] = np.sqrt(
            (120 - df["action1_end_x"])**2 + 
            (40 - df["action1_end_y"])**2
        )
        df["key_pass_enters_box"] = (
            (df["action1_end_x"] >= 102) & 
            (df["action1_end_y"] >= 18) & 
            (df["action1_end_y"] <= 62)
        ).astype(int)
    ...
    # Unique contributors
    player_cols = [f"action{i}_player_id" for i in range(1, k + 1) if f"action{i}_player_id" in df.columns]
    if player_cols:
        df["num_contributors"] = df[player_cols].apply(
...
def _log_sequence_stats(df: pd.DataFrame) -> None:
    ...
    logger.info(f"  Action types: Passes={df['num_passes'].sum()}, "
...
def build_action_level_dataset(
    ...
    for _, seq in sequence_df.iterrows():
        ...
        for pos in range(1, k + 1):
            ...
            action["distance"] = np.sqrt(
                (action["end_x"] - action["start_x"])**2 +
                (action["end_y"] - action["start_y"])**2
            ) if action["end_x"] and action["start_x"] else 0
            
            action["x_progress"] = (
                action["end_x"] - action["start_x"]
            ) if action["end_x"] and action["start_x"] else 0
            
            action["distance_to_goal"] = np.sqrt(
                (120 - action["end_x"])**2 + 
                (40 - action["end_y"])**2
            ) if action["end_x"] else None
