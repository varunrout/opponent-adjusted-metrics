...
def add_opponent_profiles(
    df: pd.DataFrame, profiles: pd.DataFrame, opponent_mapping: pd.DataFrame
) -> pd.DataFrame:
    ...
    df["shot_zone"] = df.apply(
        lambda r: assign_shot_zone(r["shot_x"], r["shot_y"]) if pd.notna(r["shot_x"]) else None,
        axis=1,
    )
...
def add_game_state(df: pd.DataFrame, goals_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["score_differential"] = df.apply(
        lambda r: score_lookup.get((r["match_id"], r["team_id"], int(r["sequence_minute"])), 0),
        axis=1,
    )
...
def add_match_defensive_stats(
    df: pd.DataFrame, defensive_stats: pd.DataFrame, opponent_mapping: pd.DataFrame
) -> pd.DataFrame:
    ...
    opp_stats = defensive_stats[
        ["match_id", "defending_team_id", "blocks", "interceptions", "clearances", "pressures"]
    ].copy()
    opp_stats.columns = [
        "match_id",
        "opponent_team_id",
        "opponent_match_blocks",
        "opponent_match_interceptions",
        "opponent_match_clearances",
        "opponent_match_pressures",
    ]
    ...
    logger.info(
        f"  Mean opponent defensive actions: {df['opponent_match_defensive_actions'].mean():.1f}"
    )
