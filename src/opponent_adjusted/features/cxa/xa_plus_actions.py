...
class XAPlusActionModel:
    ...
    def _prepare_features(self, actions_df: pd.DataFrame) -> np.ndarray:
        ...
        # Feature columns
        feature_cols = [
            'end_x', 'end_y', 'distance_to_goal',
            'is_cross', 'is_through_ball', 'under_pressure',
...
def compute_xa_plus_actions(
    ...
    goal_seq_indices = df[df['is_goal'] == True].index
    ...
    if len(goal_actions) > 0:
        goal_actions['xa_plus'] = goal_actions.groupby('sequence_idx')['score'].transform(_softmax_group)
        ...
        for seq_idx, group in goal_actions.groupby('sequence_idx'):
            for action_num, weight in zip(group['action_num'].astype(int), group['xa_plus'].astype(float)):
