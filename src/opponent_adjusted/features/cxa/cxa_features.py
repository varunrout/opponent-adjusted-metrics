...
def add_pass_difficulty_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Danger zone entry
    df["enters_box"] = ((df["end_x"] >= 102) & (df["end_y"] >= 18) & (df["end_y"] <= 62)).astype(
        int
    )

    # Half-space entry (channels beside the box)
    df["enters_half_space"] = (
        (df["end_x"] >= 90) & ((df["end_y"] <= 30) | (df["end_y"] >= 50))
    ).astype(int)

    # Zone 14 entry (central danger zone)
    df["enters_zone14"] = (
        (df["end_x"] >= 90) & (df["end_x"] <= 102) & (df["end_y"] >= 30) & (df["end_y"] <= 50)
    ).astype(int)
...
def add_pressure_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["pressure_in_own_third"] = (df["under_pressure_binary"] & (df["start_x"] < 40)).astype(int)

    df["pressure_in_final_third"] = (df["under_pressure_binary"] & (df["start_x"] >= 80)).astype(
        int
    )
...
def add_sequence_buildup_features(df: pd.DataFrame, k: int = 3) -> pd.DataFrame:
    ...
    first_start_cols = [
        f"pass{i}_start_x" for i in range(1, k + 1) if f"pass{i}_start_x" in df.columns
    ]
    ...
    if shot_x_col in df.columns and first_start_cols:
        ...
        total_progress = (
            df["sequence_total_progress"].clip(lower=1)
            if "sequence_total_progress" in df.columns
            else 1
        )
...
def add_receive_zone_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Shot location zones
    df["shot_in_box"] = ((shot_x >= 102) & (shot_y >= 18) & (shot_y <= 62)).astype(int)

    df["shot_in_six_yard"] = ((shot_x >= 114) & (shot_y >= 30) & (shot_y <= 50)).astype(int)

    # Distance from goal
    df["shot_distance_to_goal"] = np.sqrt((120 - shot_x) ** 2 + (40 - shot_y) ** 2)

    # Angle to goal
    df["shot_angle_to_goal"] = np.abs(np.arctan2(40 - shot_y, 120 - shot_x)) * 180 / np.pi

    # Central vs wide shot
    df["shot_central"] = ((shot_y >= 25) & (shot_y <= 55)).astype(int)
...
def add_match_state_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["game_phase"] = pd.cut(
        df[minute_col], bins=[0, 30, 60, 120], labels=["early", "mid", "late"]
    )
...
def get_completion_model_features() -> list:
    ...
    return [
        ...
        "is_progressive",
        # Context
        "under_pressure_binary",
        "pressure_in_final_third",
        # Spatial
        "start_x",
        "start_y",
        "xt_delta",
...
def get_shot_creation_model_features() -> list:
    ...
    return [
        ...
        "sequence_avg_xt",
        "sequence_max_xt",
        "sequence_total_progress",
        "sequence_directness",
        "sequence_progressive_count",
        "sequence_into_box_count",
        # Pass types
        "sequence_cross_count",
        "sequence_through_ball_count",
        # Sequence structure
