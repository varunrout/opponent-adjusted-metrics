...
from opponent_adjusted.db.models import Event, Shot, Match, PassEvent, CarryEvent
...
def build_shot_action_windows(
    ...
    try:
        ...
        logger.info(f"  Shots with ≥1 action: {shots_with_actions:,} ({100*shots_with_actions/len(result):.1f}%)")
...
def _load_events(session: Session, match_ids: List[int]) -> pd.DataFrame:
    ...
    all_events = [dict(r) for r in passes] + [dict(r) for r in carries] + [dict(r) for r in dribbles]
...
def _build_single_window(
    ...
    if num_actions > 0:
        ...
        for i, evt in enumerate(evt_values, start=1):
            ...
            row[f"{prefix}under_pressure"] = bool(evt["under_pressure"]) if pd.notna(evt["under_pressure"]) else False
    ...
    for i in range(num_actions + 1, max_actions + 1):
        prefix = f"action{i}_"
        for col in ["type", "player_id", "start_x", "start_y", "end_x", "end_y",
                    "distance_to_goal", "angle_to_goal", "under_pressure", "seconds_to_shot",
                    "is_pass", "is_carry", "is_dribble", "is_into_box"]:
