...
def build_player_features(
    ...
    player_stats = passes_df.groupby(["player_id", name_col]).agg(
        # Volume
        ...
        # Progressive
        progressive_passes=("is_progressive", "sum") if "is_progressive" in passes_df.columns else ("pass_id", "count"),
        
        # Into box
        into_box_passes=("is_into_box", "sum") if "is_into_box" in passes_df.columns else ("pass_id", "count"),
        
        # Crosses
        crosses=("is_cross", "sum") if "is_cross" in passes_df.columns else ("pass_id", "count"),
        
        # Through balls
        through_balls=("is_through_ball", "sum") if "is_through_ball" in passes_df.columns else ("pass_id", "count"),
        
        # xT features
        total_xt_gained=("xt_delta", "sum") if "xt_delta" in passes_df.columns else ("pass_id", "count"),
        mean_xt_delta=("xt_delta", "mean") if "xt_delta" in passes_df.columns else ("pass_id", "count"),
        max_xt_delta=("xt_delta", "max") if "xt_delta" in passes_df.columns else ("pass_id", "count"),
        ...
        key_passes=("is_key_pass", "sum") if "is_key_pass" in passes_df.columns else ("pass_id", "count"),
        second_assists=("is_second_assist", "sum") if "is_second_assist" in passes_df.columns else ("pass_id", "count"),
    ...
    player_stats["cross_rate"] = (
        player_stats["crosses"] / player_stats["total_passes"]
    ).fillna(0)
...
def build_team_features(
    ...
    team_stats = possessions_df.groupby("team_id").agg(
        # Volume
        ...
        mean_poss_duration=("duration_seconds", "mean") if "duration_seconds" in possessions_df.columns else ("num_passes", "mean"),
        
        # xT
        total_xt_gained=("total_xt_gained", "sum") if "total_xt_gained" in possessions_df.columns else ("num_passes", "count"),
        mean_xt_per_poss=("total_xt_gained", "mean") if "total_xt_gained" in possessions_df.columns else ("num_passes", "mean"),
        
        # Shot creation
        possessions_with_shot=("ended_in_shot", "sum") if "ended_in_shot" in possessions_df.columns else ("num_passes", "count"),
        possessions_with_goal=("ended_in_goal", "sum") if "ended_in_goal" in possessions_df.columns else ("num_passes", "count"),
        total_shot_xg=("shot_xg", "sum") if "shot_xg" in possessions_df.columns else ("num_passes", "count"),
        
        # Pass types in possessions
        progressive_passes=("num_progressive", "sum") if "num_progressive" in possessions_df.columns else ("num_passes", "count"),
        into_box_passes=("num_into_box", "sum") if "num_into_box" in possessions_df.columns else ("num_passes", "count"),
        crosses=("num_crosses", "sum") if "num_crosses" in possessions_df.columns else ("num_passes", "count"),
        
        # Zone progression
        completion_rate=("completion_rate", "mean") if "completion_rate" in possessions_df.columns else ("num_passes", "mean"),
    ).reset_index()
    ...
    team_stats["xt_per_pass"] = (
        team_stats["total_xt_gained"] / team_stats["total_passes"]
    ).fillna(0)
    ...
    team_stats["cross_rate"] = (
        team_stats["crosses"] / team_stats["total_passes"]
    ).fillna(0)
