...
def add_line_breaking_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["breaks_into_attack"] = ((df["start_zone_idx"] < 2) & (df["end_zone_idx"] == 2)).astype(int)

    # Into box
    df["start_in_box"] = _in_box(df["start_x"], df["start_y"])
    df["end_in_box"] = _in_box(df["end_x"], df["end_y"])
    df["breaks_into_box"] = ((df["start_in_box"] == 0) & (df["end_in_box"] == 1)).astype(int)
...
def _classify_zone(x_series: pd.Series) -> pd.Series:
    ...
    return pd.Series(np.select(conditions, choices, default="midfield"), index=x_series.index)


def _in_box(x_series: pd.Series, y_series: pd.Series) -> pd.Series:
    """Check if coordinates are inside the penalty box."""
    return ((x_series >= BOX_X_MIN) & (y_series >= BOX_Y_MIN) & (y_series <= BOX_Y_MAX)).astype(int)
...
def add_space_creation_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_switch"] = ((df["lateral_distance"] > 30) & (df["action_type"] == "Pass")).astype(int)
    ...
    df["enters_zone14"] = (
        (df["end_x"] >= 80) & (df["end_x"] <= 102) & (df["end_y"] >= 30) & (df["end_y"] <= 50)
    ).astype(int)
...
def add_final_third_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["in_final_third"] = (
        (df["start_in_final_third"] == 1) & (df["end_in_final_third"] == 1)
    ).astype(int)

    # Distance to goal at end
    df["end_distance_to_goal"] = np.sqrt((120 - df["end_x"]) ** 2 + (40 - df["end_y"]) ** 2)

    # Angle to goal at end
    df["end_angle_to_goal"] = np.abs(np.arctan2(40 - df["end_y"], 120 - df["end_x"])) * 180 / np.pi
...
def _estimate_bypass_proxy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    distance = np.sqrt((df["end_x"] - df["start_x"]) ** 2 + (df["end_y"] - df["start_y"]) ** 2)
...
def add_cross_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["cross_to_near_post"] = (crosses & (df["end_y"] < 40) & (df["end_in_box"] == 1)).astype(int)

    df["cross_to_far_post"] = (crosses & (df["end_y"] > 40) & (df["end_in_box"] == 1)).astype(int)

    # Cross from deep vs cutback
    df["cross_from_byline"] = (crosses & (df["start_x"] >= 110)).astype(int)

    df["cross_cutback"] = (
        crosses & (df["start_x"] >= 102) & (df["end_x"] < df["start_x"])  # Ball comes back
    ).astype(int)

    # Cross height (if available)
    if "pass_height" in df.columns:
        df["cross_is_low"] = (crosses & (df["pass_height"] == "Low")).astype(int)

        df["cross_is_high"] = (crosses & (df["pass_height"] == "High")).astype(int)
...
def add_through_ball_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["through_ball_into_channel"] = (
        through_balls & (df["end_x"] >= 90) & ((df["end_y"] < 25) | (df["end_y"] > 55))
    ).astype(int)
    ...
    through_ball_distance = np.where(
        through_balls,
        np.sqrt((df["end_x"] - df["start_x"]) ** 2 + (df["end_y"] - df["start_y"]) ** 2),
        0,
    )
