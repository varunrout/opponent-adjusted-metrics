"""Train a pressure influence model using distance/angle buckets."""
...
BUCKET_COL = "pressure_bucket"
BUCKET_FEATURE = "pressure_encoder"
NUMERIC_FEATURES = ["statsbomb_xg"]
GROUP_COLS = ["pressure_state", "distance_bin", "angle_bin"]
MIN_SHOTS = 25
DISTANCE_BINS = [0.0, 12.0, 18.0, 24.0, float("inf")]
DISTANCE_LABELS = ["0-12", "12-18", "18-24", "24+"]
ANGLE_BINS = [0.0, 20.0, 40.0, float("inf")]
ANGLE_LABELS = ["0-20", "20-40", "40+"]


def _prepare(df: pd.DataFrame) -> pd.DataFrame:
    subset = df.dropna(subset=["pressure_state", "shot_distance", "shot_angle"]).copy()
    if subset.empty:
        raise ValueError("No rows with pressure + geometry fields available for training.")
    subset["pressure_state"] = subset["pressure_state"].astype("object").fillna("Unknown")
    subset["distance_bin"] = pd.cut(
        subset["shot_distance"],
        bins=DISTANCE_BINS,
        labels=DISTANCE_LABELS,
        include_lowest=True,
        right=False,
    )
    subset["angle_bin"] = pd.cut(
        subset["shot_angle"],
        bins=ANGLE_BINS,
        labels=ANGLE_LABELS,
        include_lowest=True,
    ...
    subset = subset.dropna(subset=["distance_bin", "angle_bin"])
    subset["distance_bin"] = subset["distance_bin"].astype(str)
    subset["angle_bin"] = subset["angle_bin"].astype(str)
...
def main() -> None:
    ...
    model_path = out_dir / "pressure_influence_model.joblib"
    metrics_path = out_dir / "pressure_influence_model_metrics.json"
    summary_path = out_dir / "pressure_influence_modeled.csv"
