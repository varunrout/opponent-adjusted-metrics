...
class CxTModel:
    ...
    def load(cls, model_dir: Path) -> "CxTModel":
        ...
        return cls(
            ...
            completion_features=config.get("completion_features", config.get("feature_columns", [])),
...
def train_cxt_model(
    ...
    logger.info(f"  Completion AUC: {completion_metrics['auc_mean']:.3f} ± {completion_metrics['auc_std']:.3f}")
...
def evaluate_cxt_model(
    ...
    pred_xt_gain = model.predict_xt_gain(df)
