...
class UltimateCxAModel:
    ...
    def aggregate_players(
        ...
        logger.info(
            f"  Top player: {players_df.iloc[0]['player_name']} ({players_df.iloc[0]['total_cxa']:.2f} cXA)"
        )
    ...
    def save(self, path: Path) -> None:
        ...
        metadata = {
            ...
            "feature_importance": (
                self.feature_importance if hasattr(self, "feature_importance") else {}
            ),
        }
    ...
    for _, seq in sequences_df.iterrows():
        ...
        for pos in range(1, min(num_actions + 1, max_actions + 1)):
            ...
            action_row = {
                ...
                "minute": seq.get(
                    "sequence_minute", seq.get("shot_minute", seq.get(f"{prefix}minute"))
                ),
            ...
            if pd.notna(action_row["end_x"]) and pd.notna(action_row["end_y"]):
                ...
                action_row["enters_half_space"] = int(
                    88 < action_row["end_x"] < 120
                    and (18 < action_row["end_y"] < 30 or 50 < action_row["end_y"] < 62)
                )

            # Progression
            if pd.notna(action_row.get("start_x")) and pd.notna(action_row.get("end_x")):
