...
def compute_lift_table(
    ...
    # Aggregate
    agg = (
        df_temp.groupby("_bin", observed=True)
        .agg(
...
def _plot_slice_analysis(
    ...
    for i, feature in enumerate(available_features[:4]):
        ...
        # Color by lift value
        colors = [
            "#d62728" if v < 0.95 else "#2ca02c" if v > 1.05 else "#7f7f7f" for v in feat_df["lift"]
        ]
    ...
    colors_range = [
        "#2ca02c" if r > 0.1 else "#ff7f0e" if r > 0.05 else "#d62728"
    ...
    groups = (
        signal_df.groupby("slice_group")["lift_ratio"].agg(["mean", "max", "min"]).reset_index()
    )
    ...
    if zone_feature:
        ...
        try:
            ...
            pivot = zone_df.pivot_table(
                index="third", columns="width", values="lift", aggfunc="mean"
            )
    ...
    if not macro_zone_df.empty and len(macro_zone_df) > 1:
        ...
        colors = [
            "#2ca02c" if v > 1.05 else "#d62728" if v < 0.95 else "#7f7f7f"
            for v in zone_sorted["lift"]
        ]

        bars = ax.bar(
            range(len(zone_sorted)), zone_sorted["lift"], color=colors, edgecolor="black", alpha=0.8
        )
