...
def run_phase2_eda(df: pd.DataFrame, output_dir: Path) -> Dict[str, Any]:
    ...
    # Completion by zone
    completion_by_zone = (
        passes.groupby("macro_zone_start")
        .agg(
    ...
    for zone, row in completion_by_zone.iterrows():
        logger.info(
            f"  Zone {zone}: {row['completion_rate']*100:.1f}% ({row['total_passes']:,} passes)"
        )
    ...
    ax.barh(range(len(top_outcomes)), top_outcomes["mean"], color=colors, alpha=0.7)
    ...
    logger.info(
        f"  Final third entries: {final_third_entries:,} ({final_third_entries/len(df)*100:.2f}%)"
    )
    logger.info(
        f"  Penalty area entries: {penalty_area_entries:,} ({penalty_area_entries/len(df)*100:.2f}%)"
    )
    ...
    for action, row in entries_by_type.iterrows():
        logger.info(
            f"  {action}: {row['final_third_rate']*100:.1f}% final third, {row['penalty_area_rate']*100:.2f}% pen area"
        )
