...
def run_phase3_eda(
    ...
    by_opponent = df.groupby("opponent_id").agg(
    ...
    by_opponent["actions_per_match"] = (by_opponent["total_actions"] / by_opponent["matches"]).round(1)
    ...
    logger.info(f"  Mean actions per match vs opponent: {by_opponent['actions_per_match'].mean():.1f}")
    ...
    tier_analysis = df.groupby("opponent_pressure_tier").agg(
        total_actions=("event_id", "count"),
        mean_xt_delta=("xt_delta", "mean"),
        progressive_rate=("is_progressive", "mean"),
        under_pressure_rate=("under_pressure", "mean"),
    ...
    for tier, row in tier_analysis.iterrows():
        logger.info(f"  {tier}: {row['total_actions']:,} actions, "
    ...
    if "High" in tier_analysis.index and "Low" in tier_analysis.index:
        xt_signal = tier_analysis.loc["Low", "mean_xt_delta"] - tier_analysis.loc["High", "mean_xt_delta"]
        results["pressure_tier_xt_signal"] = xt_signal
        logger.info(f"\n  Signal Check: Low vs High pressure xT diff = {xt_signal:.6f}")
        if xt_signal > 0:
            logger.info(f"  ✓ Expected direction (more xT vs low-pressure opponents)")
        else:
            logger.info(f"  ⚠ Unexpected direction (investigate)")
    ...
    if opponent_profiles is not None and len(opponent_profiles) > 0:
        ...
        if "global_rating" in global_profiles.columns:
            ...
            strength_analysis = df.groupby("opponent_strength_tier").agg(
                total_actions=("event_id", "count"),
                mean_xt_delta=("xt_delta", "mean"),
                progressive_rate=("is_progressive", "mean"),
            ).round(4)
            ...
            for tier, row in strength_analysis.iterrows():
                logger.info(f"  {tier}: xT={row['mean_xt_delta']:.4f}, prog={row['progressive_rate']*100:.1f}%")
    ...
    for loc, row in home_away.iterrows():
        logger.info(f"  {loc}: {row['total_actions']:,} actions, "
                   f"xT={row['mean_xt_delta']:.4f}, "
                   f"pressure={row['under_pressure_rate']*100:.1f}%")
    
    if "Home" in home_away.index and "Away" in home_away.index:
        home_advantage = home_away.loc["Home", "mean_xt_delta"] - home_away.loc["Away", "mean_xt_delta"]
    ...
    by_competition = df.groupby("competition_id").agg(
        total_actions=("event_id", "count"),
        mean_xt_delta=("xt_delta", "mean"),
        progressive_rate=("is_progressive", "mean"),
        matches=("match_id", "nunique"),
    ...
    for comp, row in by_competition.iterrows():
        logger.info(f"    Competition {comp}: {row['matches']} matches, xT={row['mean_xt_delta']:.4f}")
