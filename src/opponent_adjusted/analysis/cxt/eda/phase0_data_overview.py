...
def run_phase0_eda(df: pd.DataFrame, output_dir: Path) -> Dict[str, Any]:
    ...
    logger.info("\nxT Statistics:")
    logger.info(
        f"  - start_xt: mean={xt_stats['start_xt']['mean']:.4f}, std={xt_stats['start_xt']['std']:.4f}"
    )
    logger.info(
        f"  - end_xt: mean={xt_stats['end_xt']['mean']:.4f}, std={xt_stats['end_xt']['std']:.4f}"
    )
    logger.info(
        f"  - xt_delta: mean={xt_stats['xt_delta']['mean']:.6f}, std={xt_stats['xt_delta']['std']:.4f}"
    )
