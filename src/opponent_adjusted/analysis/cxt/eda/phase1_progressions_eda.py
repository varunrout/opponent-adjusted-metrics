...
def run_phase1_eda(df: pd.DataFrame, output_dir: Path) -> Dict[str, Any]:
    ...
    ax1.axvline(
        ...
        color="green",
        linestyle="--",
        label=f"Mean: {df['xt_delta'].mean():.4f}",
    )
    ...
    transition_matrix = pd.crosstab(
        df["macro_zone_start"], df["macro_zone_end"], normalize="index"
    ).round(3)
    ...
    sns.heatmap(transition_matrix, annot=True, fmt=".2f", cmap="YlOrRd", ax=ax)
    ...
    pressure_xt_diff = (
        pressure_analysis.loc[True, "mean_xt_delta"] - pressure_analysis.loc[False, "mean_xt_delta"]
    )
    ...
    start_zone_counts = (
        df.groupby(["start_zone_x", "start_zone_y"]).size().reset_index(name="count")
    )
    ...
    ax2 = axes[1]
    im2 = ax2.imshow(
        xt_heatmap, cmap="RdYlGn", aspect="auto", origin="lower", vmin=-0.02, vmax=0.02
    )
