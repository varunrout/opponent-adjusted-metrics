...
def analyze_assist_rate_by_pass_type(passes: pd.DataFrame) -> pd.DataFrame:
    ...
    for col, label in pass_types.items():
        ...
        subset = passes[passes[col].fillna(False).astype(bool)]
        total = len(subset)
...
def analyze_assist_rate_by_zone(passes: pd.DataFrame) -> pd.DataFrame:
    ...
    zone_stats = (
        passes.groupby("zone").agg({"pass_id": "count", "is_assist": ["sum", "mean"]}).reset_index()
    )
...
def analyze_assist_rate_by_pressure(passes: pd.DataFrame) -> pd.DataFrame:
    ...
    pressure_stats = (
        passes.groupby("under_pressure")
        .agg({"pass_id": "count", "is_assist": ["sum", "mean"]})
        .reset_index()
    )
    ...
    pressure_stats["pressure_state"] = pressure_stats["under_pressure"].map(
        {True: "Under Pressure", False: "Open Play"}
    )
...
def analyze_sequence_lengths(sequences: pd.DataFrame, actions: pd.DataFrame) -> pd.DataFrame:
    ...
    seq_goals = sequences[sequences["is_goal"].fillna(False).astype(bool)].copy()
    action_goals = actions[actions["is_goal"].fillna(False).astype(bool)].copy()
...
def plot_assist_rate_heatmap(passes: pd.DataFrame, out_path: Path) -> None:
    ...
    heatmap_data = (
        passes.groupby(["x_bin", "y_bin"])
        .agg({"is_assist": ["sum", "count", "mean"]})
        .reset_index()
    )
    ...
    pivot = heatmap_data.pivot_table(
        values="assist_rate", index="y_bin", columns="x_bin", fill_value=0
    )
...
def write_markdown_report(
    ...
    for seq_type in sequence_stats["sequence_type"].unique():
        ...
        for _, row in subset.iterrows():
            lines.append(
                f"- {int(row['length'])} actions: {int(row['count'])} goals ({row['percentage']:.1f}%)"
            )
        lines.append("")
    ...
    lines.append(
        f"- **Highest assist rate:** {best_type['pass_type']} ({best_type['assist_rate']*100:.2f}%)"
    )
    ...
    if not sequence_stats.empty:
        ...
        lines.append(
            f"- **Most common goal sequence:** {int(most_common['length'])} actions ({int(most_common['count'])} goals)"
        )
