...
def plot_credit_distribution(scored_actions: pd.DataFrame, output_dir: Path):
    ...
    axes[0, 0].axvline(
        credits.mean(), color="red", linestyle="--", label=f"Mean: {credits.mean():.4f}"
    )
...
def plot_position_decay_curve(scored_actions: pd.DataFrame, output_dir: Path):
    ...
    pos_stats = (
        scored_actions.groupby("position_in_window")
        .agg(
    ...
    axes[1].plot(
        pos_stats["position_in_window"],
        pos_stats["mean_credit"],
        marker="o",
        linewidth=2,
        color="coral",
    )
...
def plot_credit_by_action_type(scored_actions: pd.DataFrame, output_dir: Path):
    ...
    type_stats = (
        scored_actions.groupby("action_type")
        .agg(
            n_actions=("cxa_xg", "count"),
            total_credit=("cxa_xg", "sum"),
            mean_credit=("cxa_xg", "mean"),
    ...
    # 1. Action count
    axes[0].pie(
        type_stats["n_actions"],
        labels=type_stats["action_type"],
        autopct="%1.1f%%",
        colors=["steelblue", "coral", "green"][: len(type_stats)],
    )
    axes[0].set_title("Action Type Distribution")

    # 2. Credit share
    axes[1].pie(
        type_stats["total_credit"],
        labels=type_stats["action_type"],
        autopct="%1.1f%%",
        colors=["steelblue", "coral", "green"][: len(type_stats)],
    )
...
def plot_top_contributors(scored_actions: pd.DataFrame, output_dir: Path):
    ...
    if "player_name" in scored_actions.columns:
        player_totals = (
            scored_actions.groupby("player_name")["cxa_xg"].sum().sort_values(ascending=False)
        )
...
def plot_xg_calibration_check(scored_actions: pd.DataFrame, shots: pd.DataFrame, output_dir: Path):
    ...
    merged = credit_per_shot.merge(
        shots[["shot_id", "statsbomb_xg", "is_goal"]], on="shot_id", how="left"
    )
...
def plot_credit_heatmap(scored_actions: pd.DataFrame, output_dir: Path):
    ...
    h1 = axes[0].hist2d(scored_actions["end_x"], scored_actions["end_y"], bins=20, cmap="YlOrRd")
    ...
    h2 = axes[1].hist2d(
        scored_actions["end_x"],
        scored_actions["end_y"],
        bins=[x_bins, y_bins],
