...
def analyze_action_counts(df: pd.DataFrame, output_dir: Path):
    ...
    axes[0, 0].axvline(num_actions.mean(), color='red', linestyle='--',
                       label=f"Mean: {num_actions.mean():.2f}")
...
def analyze_action_types_by_position(df: pd.DataFrame, output_dir: Path):
    ...
    bottom_pass = np.zeros(len(position_df))
    bottom_carry = position_df['pass_pct'].values
    
    axes[0, 1].bar(x, position_df['pass_pct'], width, label='Pass', color='steelblue')
    axes[0, 1].bar(x, position_df['carry_pct'], width, bottom=bottom_carry, label='Carry', color='coral')
    if 'dribble_pct' in position_df.columns:
        bottom_dribble = bottom_carry + position_df['carry_pct'].values
        axes[0, 1].bar(x, position_df['dribble_pct'], width, bottom=bottom_dribble, label='Dribble', color='green')
...
def analyze_shot_features(df: pd.DataFrame, output_dir: Path):
    ...
    if 'shot_xg' in df.columns:
        axes[1, 0].hist(df['shot_xg'].dropna(), bins=50, edgecolor='black', alpha=0.7, color='green')
...
def analyze_action_features_by_position(df: pd.DataFrame, output_dir: Path):
    ...
    if 'end_x_mean' in position_features_df.columns:
        axes[0, 0].plot(position_features_df['position'], position_features_df['end_x_mean'], 
    ...
    if 'end_y_mean' in position_features_df.columns:
        axes[0, 1].plot(position_features_df['position'], position_features_df['end_y_mean'],
    ...
    if 'start_x_mean' in position_features_df.columns:
        axes[1, 0].plot(position_features_df['position'], position_features_df['start_x_mean'],
    ...
    if 'start_y_mean' in position_features_df.columns:
        axes[1, 1].plot(position_features_df['position'], position_features_df['start_y_mean'],
