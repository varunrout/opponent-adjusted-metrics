...
def plot_xg_distribution(df: pd.DataFrame, output_dir: Path):
    ...
    axes[0, 0].axvline(
        xg.median(), color="green", linestyle="--", label=f"Median: {xg.median():.3f}"
    )
    ...
    axes[1, 0].hist(
        non_goals, bins=30, alpha=0.6, label=f"Non-goal (n={len(non_goals):,})", color="steelblue"
    )
...
def plot_xg_calibration(df: pd.DataFrame, output_dir: Path):
    ...
    calibration = (
        df.groupby("xg_bin", observed=True)
        .agg(
...
def plot_shot_locations(df: pd.DataFrame, output_dir: Path):
    ...
    conversion = (
        df_temp.groupby(["x_bin", "y_bin"], observed=True)
    ...
    im = axes[2].imshow(
        rate_matrix.values, cmap="RdYlGn", aspect="auto", origin="lower", vmin=0, vmax=0.5
    )
...
def run_phase2_eda(output_dir: Path = None) -> Dict[str, Any]:
    ...
    analyze_shot_types(shots, output_dir)
    ...
    ax.hist(
        shots[shots["is_goal"] == 0]["distance_to_goal"],
        bins=30,
        alpha=0.6,
        label="Non-goal",
        density=True,
    ...
    ax.hist(
        shots[shots["is_goal"] == 1]["distance_to_goal"],
        bins=30,
        alpha=0.6,
        label="Goal",
        density=True,
