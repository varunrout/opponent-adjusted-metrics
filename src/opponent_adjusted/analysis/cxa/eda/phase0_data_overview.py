...
def run_phase0_eda(output_dir: Path = None) -> Dict[str, Any]:
    ...
    if "sequences" in dfs:
        seq_goals = dfs["sequences"]["is_goal"].sum() if "is_goal" in dfs["sequences"].columns else len(dfs["sequences"])
    ...
    if "shots" in dfs:
        coverage = {
            ...
            "unique_players": dfs["shots"]["player_id"].nunique() if "player_id" in dfs["shots"].columns else "N/A",
        }
