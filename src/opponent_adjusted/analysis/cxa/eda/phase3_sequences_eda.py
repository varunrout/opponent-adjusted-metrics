...
def analyze_sequence_lengths(df: pd.DataFrame, output_dir: Path):
    ...
    axes[0, 0].axvline(
        seq_lengths.mean(), color="red", linestyle="--", label=f"Mean: {seq_lengths.mean():.2f}"
    )
...
def analyze_sequence_by_outcome(df: pd.DataFrame, shots: pd.DataFrame, output_dir: Path):
    ...
    merged = df[["shot_id", "num_passes_in_sequence"]].merge(
        shots[["shot_id", "is_goal"]], on="shot_id", how="left"
    )
    ...
    bins = range(1, 12)
    axes[0].hist(
        non_goals, bins=bins, alpha=0.6, label=f"Non-goals (n={len(non_goals):,})", density=True
    )
    axes[0].hist(goals, bins=bins, alpha=0.6, label=f"Goals (n={len(goals):,})", density=True)
    ...
    # 2. Box plot
    axes[1].boxplot([non_goals.dropna(), goals.dropna()], labels=["Non-goal", "Goal"])
...
def analyze_pass_by_position(df: pd.DataFrame, output_dir: Path):
    ...
    if "mean_is_progressive" in position_df.columns:
        axes[1, 1].bar(
            position_df["position"], position_df["mean_is_progressive"] * 100, color="green"
        )
...
def analyze_temporal_features(df: pd.DataFrame, output_dir: Path):
    ...
    for i, col in enumerate(temporal_cols):
        ...
        axes[i].axvline(
            series.mean(), color="red", linestyle="--", label=f"Mean: {series.mean():.1f}"
        )
