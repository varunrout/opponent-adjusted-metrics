...
def plot_lift_chart(passes: pd.DataFrame, features: List[str], output_dir: Path):
    ...
    lift = df.groupby('decile', observed=True).agg(
        count=('y', 'count'),
        assists=('y', 'sum'),
    ...
    # 2. Cumulative gains
    perfect_line = np.linspace(0, 1, len(lift) + 1)
    axes[1].plot(range(len(lift) + 1), [0] + list(lift['cumulative_pct']), 
...
def run_phase5_eda(output_dir: Path = None) -> Dict[str, Any]:
    ...
    numeric_features = ['end_x', 'end_y', 'start_x', 'start_y', 'pass_length', 
                        'xt_delta', 'end_xt', 'start_xt']
    boolean_features = ['is_cross', 'is_through_ball', 'is_into_box', 
                        'is_progressive', 'is_final_third', 'under_pressure']
