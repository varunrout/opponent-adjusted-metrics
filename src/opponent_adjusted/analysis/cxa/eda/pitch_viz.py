...
def draw_pitch(ax, pitch_color='white', line_color='black', orientation='horizontal'):
    ...
    if orientation == 'horizontal':
        ...
        # Center line
        ax.plot([pitch_length/2, pitch_length/2], [0, pitch_width], color=line_color, linewidth=1)
...
def plot_pass_map(df: pd.DataFrame, title: str = "Pass Map", 
                  alpha: float = 0.3, color: str = 'steelblue',
...
def plot_shot_map(df: pd.DataFrame, title: str = "Shot Map",
                  figsize: Tuple[int, int] = (12, 8)) -> plt.Figure:
    ...
    # Plot non-goals
    ax.scatter(non_goals['shot_x'], non_goals['shot_y'], 
               s=non_goals['statsbomb_xg'] * 500 + 20,
               c='steelblue', alpha=0.5, edgecolor='black', linewidth=0.5,
               label='No Goal')
    
    # Plot goals
    ax.scatter(goals['shot_x'], goals['shot_y'],
               s=goals['statsbomb_xg'] * 500 + 20,
               c='coral', alpha=0.8, edgecolor='black', linewidth=0.5,
               marker='*', label='Goal')
...
def plot_heatmap_on_pitch(x: np.ndarray, y: np.ndarray, 
    ...
    if weights is not None:
        hb = ax.hexbin(x, y, C=weights, gridsize=20, cmap=cmap, 
                       reduce_C_function=np.sum, alpha=0.8)
    else:
...
def plot_action_flow(df: pd.DataFrame, shot_row: pd.Series,
    ...
    for i, (_, action) in enumerate(df.iterrows()):
        ...
        ax.text(mid_x, mid_y, str(i+1), fontsize=10, ha='center', va='center',
               bbox=dict(boxstyle='circle', facecolor='white', edgecolor=color))
    
    # Plot shot
    ax.scatter(shot_row['shot_x'], shot_row['shot_y'], 
               s=200, c='red', marker='X', zorder=5, label='Shot')
...
def plot_zones_analysis(df: pd.DataFrame, value_col: str,
    ...
    zone_values = df.groupby(['x_zone', 'y_zone'], observed=True)[value_col].sum().unstack(fill_value=0)
