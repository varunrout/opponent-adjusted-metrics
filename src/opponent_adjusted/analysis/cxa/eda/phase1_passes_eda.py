...
def plot_class_imbalance(df: pd.DataFrame, target_col: str, output_dir: Path):
    ...
    axes[1].pie(counts.values, labels=[f'{k}\n({v:,}, {100*v/len(df):.2f}%)' 
                ...
                autopct='', colors=['steelblue', 'coral'])
...
def plot_spatial_heatmap(df: pd.DataFrame, x_col: str, y_col: str, 
...
def plot_boxplots_by_category(df: pd.DataFrame, numeric_col: str, 
                               category_col: str, output_dir: Path, filename: str):
...
def analyze_by_target(df: pd.DataFrame, target_col: str, 
...
def run_phase1_eda(output_dir: Path = None) -> Dict[str, Any]:
    ...
    numeric_features = ['start_x', 'start_y', 'end_x', 'end_y', 'pass_length', 
                        'pass_angle', 'start_xt', 'end_xt', 'xt_delta']
    boolean_features = ['is_cross', 'is_through_ball', 'is_into_box', 'is_progressive',
                        'is_complete', 'under_pressure', 'is_set_piece', 'is_final_third']
    ...
    if "is_assist" not in passes.columns:
        ...
        for col in ['pass1_id', 'pass2_id', 'pass3_id']:
            if col in sequences.columns:
                ...
                if col == 'pass1_id':
                    goal_seqs = sequences[sequences['is_goal'] == True] if 'is_goal' in sequences.columns else sequences
    ...
    # 6. Spatial Analysis
    logger.info("\n--- 6. Spatial Analysis ---")
    plot_spatial_heatmap(passes, 'end_x', 'end_y', 'All Pass Destinations', 
                         output_dir, 'spatial_all_passes.png')
    
    if 'is_assist' in passes.columns:
        assists = passes[passes['is_assist'] == 1]
        if len(assists) > 0:
            plot_spatial_heatmap(assists, 'end_x', 'end_y', 'Assist Pass Destinations',
                                output_dir, 'spatial_assists.png')
    ...
    if 'pass_type' in passes.columns:
        by_type = passes.groupby('pass_type').agg(
            total=('pass_id', 'count'),
    ...
    if 'end_x' in passes.columns:
        passes['zone'] = pd.cut(passes['end_x'], bins=[0, 40, 80, 120], 
                                 labels=['Defensive', 'Middle', 'Attacking'])
        by_zone = passes.groupby('zone').agg(
            total=('pass_id', 'count'),
            assists=('is_assist', 'sum')
        ).reset_index()
