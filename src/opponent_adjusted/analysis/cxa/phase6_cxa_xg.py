...
def melt_to_actions(windows_df: pd.DataFrame, max_actions: int = 5) -> pd.DataFrame:
    ...
    for _, row in windows_df.iterrows():
        ...
        for i in range(1, min(num_actions, max_actions) + 1):
            ...
            end_x = (
                float(row.get(f"{prefix}end_x", 60))
                if pd.notna(row.get(f"{prefix}end_x"))
                else 60.0
            )
            end_y = (
                float(row.get(f"{prefix}end_y", 40))
                if pd.notna(row.get(f"{prefix}end_y"))
                else 40.0
            )
...
def score_and_allocate(
    actions_df: pd.DataFrame, scorer, features: list, mode: str = "xG"
) -> pd.DataFrame:
...
def player_leaderboard(credits_df: pd.DataFrame, mode: str = "xG") -> pd.DataFrame:
    ...
    agg = (
        credits_df.groupby(["player_id", "player_name"])
        .agg(
...
def run_phase6_analysis():
    ...
    calibration = pd.DataFrame(
        [
            {
                "metric": "Total xG (shots w/ actions)",
                "expected": total_xg,
                "attributed": attributed_xg,
                "diff": attributed_xg - total_xg,
            },
            {
                "metric": "Total Goals (shots w/ actions)",
                "expected": total_goals,
                "attributed": attributed_goals,
                "diff": attributed_goals - total_goals,
            },
        ]
    )
    ...
    position_summary = (
        cxa_xg.groupby("action_position")
        .agg(
    ...
    position_summary["pct_of_total"] = (
        100 * position_summary["total_cxa_xg"] / position_summary["total_cxa_xg"].sum()
    )
...
def _generate_report(
    ...
    pass_pct = (
        type_summary[type_summary["action_type"] == "Pass"]["pct_xG"].iloc[0]
        if "Pass" in type_summary["action_type"].values
        else 0
    )
    carry_pct = (
        type_summary[type_summary["action_type"] == "Carry"]["pct_xG"].iloc[0]
        if "Carry" in type_summary["action_type"].values
        else 0
    )
    dribble_pct = (
        type_summary[type_summary["action_type"] == "Dribble"]["pct_xG"].iloc[0]
        if "Dribble" in type_summary["action_type"].values
        else 0
    )
