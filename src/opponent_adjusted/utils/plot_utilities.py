"""Common plotting utilities and style configuration.

This module centralizes colors, figure sizes, fonts, and export helpers so
all analysis and modeling code produces consistent, publication-quality plots.

Merged from the former ``analysis.plot_utilities`` and ``modeling.plot_utilities``
...
def metrics_map_to_frame(
    metrics_map: Mapping[str, Mapping[str, float]], metric_keys: list[str]
) -> pd.DataFrame:
