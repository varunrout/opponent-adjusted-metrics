...
def build_opponent_profiles(shot_features_df: pd.DataFrame, session) -> pd.DataFrame:
    ...
    Returns:
        DataFrame with one row per (team, zone) combination
    """
    logger.info("Building opponent defensive profiles...")
    
    df = shot_features_df.copy()
    
    # Need opponent_team_id and xG
...
def run_pipeline(competition_id: int = None) -> dict:
    ...
    Args:
        competition_id: Filter to specific competition (default: None = all competitions)
    ...
    with get_session() as session:
        ...
        if not opponent_profiles_df.empty:
            outputs["opponent_profiles"] = save_parquet(opponent_profiles_df, "opponent_profiles", output_dir)
