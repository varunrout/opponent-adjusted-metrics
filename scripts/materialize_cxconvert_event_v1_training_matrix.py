"""Materialize the CxA P_convert (event-only) training matrix.

Population: every row where `y_create = TRUE` in
`oam_features.cxa_event_v1_training_matrix` (P_create's own already-validated
key_pass_id-based label, not re-derived -- see
docs/cxa_convert_data_feasibility_audit.md section 1 for the confirmed 1:1
key_pass_id -> shot join this relies on). Target `y_goal = (shots.outcome_name =
'Goal')`, confirmed field/values per the audit doc section 2.

Grain: one row per Y_create=TRUE pass (== one row per its uniquely-matched shot, since
the join is 1:1). Carries over every P_create pass-context candidate feature
unfiltered, adds the shot's own attributes, and adds shot freeze-frame-derived
GK/defender positioning (near-universal coverage for this population, see audit doc
section 4c -- NOT restricted to the CxA+ track, a deliberate, documented deviation
from treating 360-derived context as CxA+-exclusive).

Excludes, per the audit's leakage findings (section 4b): `outcome_id`/`outcome_name`
(the label source), `deflected`, `saved_off_target`, `saved_to_post` (all describe
what happened to the shot, not the pre-shot situation).

No feature promotion, no filtering beyond the explicit leakage exclusions above --
full unfiltered candidate set, per the task scope (audit + pipeline only).
"""
...
FEATURE_VERSION = "cxconvert_event_v1"

NATIVE_TO_METRE_X = 105.0 / 120.0
NATIVE_TO_METRE_Y = 68.0 / 80.0

TABLE = f"{PROJECT}.{FEATURES_DATASET}.cxconvert_event_v1_training_matrix"
...
WITH creates AS (
  SELECT
    ...
    pass_outcome_name, is_completed
  FROM `{PROJECT}.{FEATURES_DATASET}.cxa_event_v1_training_matrix`
  WHERE y_create = TRUE
),
shot_base AS (
  SELECT
