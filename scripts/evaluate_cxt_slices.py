...
def define_slices(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    if "under_pressure" in df.columns:
        slices["Under Pressure"] = df[df["under_pressure"] == True]
        slices["No Pressure"] = df[df["under_pressure"] == False]
    
    # Opponent strength (if available)
    if "opponent_is_strong" in df.columns:
        slices["vs Strong Opponent"] = df[df["opponent_is_strong"] == True]
    if "opponent_is_weak" in df.columns:
        slices["vs Weak Opponent"] = df[df["opponent_is_weak"] == True]
    ...
    if "action_type" in df.columns:
        ...
        slices["Action: Pass"] = df[df["is_pass"] == True]
        slices["Action: Carry"] = df[df["is_carry"] == True]
        if "is_dribble" in df.columns:
            slices["Action: Dribble"] = df[df["is_dribble"] == True]
    ...
    if "is_late_game" in df.columns:
        slices["Late Game (75+)"] = df[df["is_late_game"] == True]
        slices["Early Game (<45)"] = df[df["is_early_game"] == True] if "is_early_game" in df.columns else df[df["minute_normalized"] < 0.5]
    
    # Progressive actions
    if "is_progressive" in df.columns:
        slices["Progressive Actions"] = df[df["is_progressive"] == True]
        slices["Non-Progressive"] = df[df["is_progressive"] == False]
    
    # Final third entries
    if "is_into_final_third" in df.columns:
        slices["Into Final Third"] = df[df["is_into_final_third"] == True]
    
    # Penalty area entries
    if "is_into_penalty_area" in df.columns:
        slices["Into Penalty Area"] = df[df["is_into_penalty_area"] == True]
...
def main():
    ...
    for slice_name, slice_df in slices.items():
        ...
        auc_str = f"{metrics.get('completion_auc', 0):.3f}" if metrics.get('completion_auc') else "N/A"
...
def generate_slice_plots(results_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    plot_df = results_df[
        results_df['status'].isin(['OK']) & 
        results_df['completion_auc'].notna()
    ].copy()
    ...
    colors = ['#d62728' if v < 0.6 else '#2ca02c' if v >= 0.8 else '#ff7f0e' 
    ...
    colors_r2 = ['#d62728' if v < 0.3 else '#2ca02c' if v >= 0.5 else '#ff7f0e' 
                 for v in r2_df['xt_gain_r2']]
    ...
    ax3.set_title('Slice Performance: Completion vs xT Gain\n(size = log(samples), color = mean CxT)')
    
    cbar = plt.colorbar(scatter, ax=ax3, label='Mean CxT')
    ...
    for idx, (cat_name, cat_slices) in enumerate(categories.items()):
        ...
        # Shorten labels
        labels = [s.replace('Action: ', '').replace('Zone: ', '').replace('Start: ', '') 
    ...
    ax5.scatter(sr_df['success_rate'], sr_df['cxt_mean'], 
               s=100, alpha=0.7, edgecolors='black')
    
    for _, row in sr_df.iterrows():
        ax5.annotate(row['slice'], (row['success_rate'], row['cxt_mean']),
    ...
    # AUC vs Sample Size
    ax6a = axes6[0]
    ax6a.scatter(valid_df['n_samples'], valid_df['completion_auc'], 
    ...
    # R² vs Sample Size
    ax6b = axes6[1]
    ax6b.scatter(valid_df['n_samples'], valid_df['xt_gain_r2'], 
                s=80, alpha=0.7, c='darkorange', edgecolors='black')
    ax6b.set_xscale('log')
    ...
    if len(cxt_df) > 0:
        ax7.scatter(cxt_df['actual_xt_mean'], cxt_df['cxt_mean'], 
        ...
        for _, row in cxt_df.iterrows():
            ax7.annotate(row['slice'], (row['actual_xt_mean'], row['cxt_mean']),
                        fontsize=7, xytext=(3, 3), textcoords='offset points')
    ...
    key_slices = ['Overall', 'Under Pressure', 'No Pressure', 'Progressive Actions', 
                 'Into Final Third', 'Late Game (75+)']
    ...
    if len(zone_df) > 0:
        ...
        ax9.bar(x - width, zone_df['completion_auc'].fillna(0), width, 
               label='Completion AUC', color='steelblue')
        ax9.bar(x, zone_df['xt_gain_r2'].fillna(0), width, 
               label='xT Gain R²', color='darkorange')
        ax9.bar(x + width, zone_df['success_rate'].fillna(0), width, 
    ...
    if len(valid_metrics) > 3:
        ...
        axes10[0].scatter([1]*len(valid_metrics), valid_metrics['completion_auc'], alpha=0.6, color='steelblue')
        ...
        axes10[1].scatter([1]*len(valid_metrics), valid_metrics['xt_gain_r2'], alpha=0.6, color='darkorange')
        ...
        axes10[2].scatter([1]*len(valid_metrics), valid_metrics['success_rate'], alpha=0.6, color='green')
    ...
    if len(prog_slices) == 2:
        ...
        bars1 = ax11.bar(x - width/2, prog_vals, width, label='Progressive', color='green', alpha=0.8)
        bars2 = ax11.bar(x + width/2, non_prog_vals, width, label='Non-Progressive', color='gray', alpha=0.8)
        ...
        ax11.text(0.5, 0.5, 'Progressive/Non-Progressive data not available', 
...
def generate_report(results_df: pd.DataFrame, df: pd.DataFrame) -> str:
    ...
    if len(pressure_rows) >= 2:
        under = pressure_rows[pressure_rows['slice'] == 'Under Pressure'].iloc[0] if len(pressure_rows[pressure_rows['slice'] == 'Under Pressure']) > 0 else None
        no = pressure_rows[pressure_rows['slice'] == 'No Pressure'].iloc[0] if len(pressure_rows[pressure_rows['slice'] == 'No Pressure']) > 0 else None
        if under is not None and no is not None:
            lines.append(f"- **Pressure impact**: Under pressure AUC={under.get('completion_auc', 'N/A'):.3f} vs No pressure AUC={no.get('completion_auc', 'N/A'):.3f}")
    ...
    overall = results_df[results_df['slice'] == 'Overall'].iloc[0] if len(results_df[results_df['slice'] == 'Overall']) > 0 else None
    if overall is not None:
        auc_pass = overall['completion_auc'] >= 0.55 if pd.notna(overall.get('completion_auc')) else False
        r2_pass = overall['xt_gain_r2'] >= 0 if pd.notna(overall.get('xt_gain_r2')) else False
        lines.append(f"| Overall Completion AUC ≥ 0.55 | {overall.get('completion_auc', 'N/A'):.3f} | {'✓' if auc_pass else '✗'} |")
        lines.append(f"| Overall xT Gain R² ≥ 0 | {overall.get('xt_gain_r2', 'N/A'):.3f} | {'✓' if r2_pass else '✗'} |")
    ...
    if warn_count == 0:
        lines.append("✓ **All slices pass acceptance criteria.** Model is ready for final integration.")
    else:
